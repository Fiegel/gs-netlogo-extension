...
to setup
  ...
  let status gs:add-singlegraph "Giant Component"
...
to init-turtles
  ...
  [ let status gs:an ?
    ...
    ifelse status
    [ output-print (word "Node " ? " added to the graph " gs:current-graph-name) ]
    [ output-print (word ? " cannot be added to the graph " gs:current-graph-name) ] ]
...
to go
  ;; if the below condition is true then we have a fully connected netword and we need to stop
  ...
  ask links
  [ set color [ color ] of end1 ] ;; recolor all edges
...
to find-all-components
  ...
  [ ;; pick a node that has not yet been explored
    ...
    ;; this variable is updated each time we explore an unexplored node
...
to add-edge
  ...
  let edge-added nobody
  
  while [ node1 = node2 or member? node2 [ link-neighbors ] of node1 ]
  [ set node1 one-of turtles
    set node2 one-of turtles ]
  
  ask node1
  [ create-link-with node2
    [ set edge-added self ] ]
  
  let status gs:ae edge-added
  
  ifelse status
  [ output-print (word "Edge " edge-added " added to the graph " gs:current-graph-name) ]
  [ output-print (word edge-added " cannot be added to the graph " gs:current-graph-name) ] 
end
